...
def place_opt(request):
	...
	pop=request.GET['pop']
	res={'station':station,'pop':pop} # tour = Tourism.objects.filter(station_name=station).values('station_name', 't_name', 'address','cate','lat', 'lon')
	tour = sort_station(station).to_dict('records')
